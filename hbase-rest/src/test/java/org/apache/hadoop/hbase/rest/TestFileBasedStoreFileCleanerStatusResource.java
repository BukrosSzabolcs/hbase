...
import org.apache.hadoop.hbase.rest.model.FileBasedStoreFileCleanerStatusModel;
...
public class TestFileBasedStoreFileCleanerStatusResource {
  ...
  @BeforeClass public static void setUpBeforeClass() throws Exception {
    ...
    context = JAXBContext.newInstance(FileBasedStoreFileCleanerStatusModel.class);
  }
  ...
  @Test public void testDeletingJunkFile() throws Exception {
    ...
    FileBasedStoreFileCleanerStatusModel model;
    Response response;
    ...
    assertEquals(model.getFileBasedFileStoreCleanerStatus().size(), 2);
    assertNotNull(model.getFileBasedFileStoreCleanerStatus()
      .get(HMaster.AGGREGATED_FILEBASED_STOREFILE_CLEANER_NAME));
    assertEquals(model.getFileBasedFileStoreCleanerStatus()
      .get(HMaster.AGGREGATED_FILEBASED_STOREFILE_CLEANER_NAME).getDeletedFiles(), 1);

    //testing JSON
    ObjectMapper jsonMapper =
      new JacksonJaxbJsonProvider().locateMapper(FileBasedStoreFileCleanerStatusModel.class,
        MediaType.APPLICATION_JSON_TYPE);
    response = client.get(namespacePath, Constants.MIMETYPE_JSON);
    assertEquals(200, response.getCode());
    model = jsonMapper.readValue(response.getBody(), FileBasedStoreFileCleanerStatusModel.class);
    assertEquals(model.getFileBasedFileStoreCleanerStatus().size(), 2);
    assertNotNull(model.getFileBasedFileStoreCleanerStatus()
      .get(HMaster.AGGREGATED_FILEBASED_STOREFILE_CLEANER_NAME));
    assertEquals(model.getFileBasedFileStoreCleanerStatus()
  ...
  private static FileBasedStoreFileCleanerStatusModel fromXML(byte[] content) throws JAXBException {
    return (FileBasedStoreFileCleanerStatusModel) context.createUnmarshaller()
